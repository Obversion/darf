...
#         Rationale:
#           * All metric-collection fits use different seeds, so each fit trains
#             on a different bootstrap sample → different work per fit → noisy
#             timing. This pass trains with a *fixed* seed so every repeat does
#             the same amount of work.
#           * Threads are pinned to 1 via threadpoolctl so the wall-clock and
#             CPU time are both dominated by actual compute.
#           * A warm-up fit of each model type is discarded so lazy imports and
#             first-call overhead never enter the measurement.
#           * Each fit is repeated n_repeats times; we report the median so a
#             single slow run can't skew the value.

#         Returns:
#             dict with the same shape the visualizer expects:
#               {
#                 'Preprocessing': float,
#                 'Training':   {'RF': .., 'XGBoost': .., 'DARF': ..},
#                 'Inference':  {'RF': .., 'XGBoost': .., 'DARF': ..},
#                 'Training_CPU':  {...},
#                 'Inference_CPU': {...},
#               }
#         """
...
#         # Fixed seed for the timing pass — does not affect metric runs.
#         timing_seed = 12345

#         def _make_and_time(family, weights, warmup=False):
#             """Fit + predict once; return (train_wall, infer_wall, train_cpu, infer_cpu)."""
...
#         # ---- Warm-up (discarded) ------------------------------------------
#         try:
#             _make_and_time('bagging', None, warmup=True)
#             _make_and_time('boosting', None, warmup=True)
#             _make_and_time(dw_family, dpw, warmup=True)
...
#         # Provisional timing from the metric runs — will be replaced by the
#         # dedicated reference measurement after all other work is done.
...
#         # 6b. Dedicated reference timing (reproducible, seeded, single-threaded)
#         #     Overwrites the provisional timing so the computational-times chart
#         #     reports a stable, representative measurement.
#         try:
